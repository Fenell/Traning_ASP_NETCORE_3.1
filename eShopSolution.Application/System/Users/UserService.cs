...
namespace eShopSolution.Application.System.Users
{
	public class UserService : IUserService
	{
		...
		public async Task<Response<string>> Authenticate(LoginRequest request)
		{
			...
			var isLogin = await _signInManager.UserManager.CheckPasswordAsync(user, request.Password);
			if (!isLogin)
			{
				return new Response<string>()
				{
		...
		public async Task<PagedResult<UserViewModel>> GetUserList(GetUserPagingRequest request)
		{
			...
			{
				query = query.Where(
					c => c.UserName.ToLower().Contains(request.Keyword.ToLower()) || c.FirstName.ToLower().Contains(request.Keyword.ToLower()) || c.PhoneNumber.Contains(request.Keyword));
			}
